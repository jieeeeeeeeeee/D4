...
namespace alpr
{
	...
	void CharacterSegmenter::segment() {
		...
		for (unsigned int lineidx = 0; lineidx < pipeline_data->textLines.size(); lineidx++)
		{
			...
			for (unsigned int i = 0; i < pipeline_data->thresholds.size(); i++)
			{
				...
				//函数fillConvexPoly绘制填充凸多边形。这个函数比函数cv: fillPoly快得多。它不仅可以填充凸多边形，
				//还可以填充任何没有自交的单调多边形，即轮廓与每条水平线(扫描线)相交最多两次的多边形
				//(不过，它的最顶端和/或底边缘可以是水平的)。
	...
	vector<Rect> CharacterSegmenter::getBestCharBoxes(Mat img, vector<Rect> charBoxes, float avgCharWidth)
	{
		...
		for (int row = 0; row < histogram.histoImg.rows; row++)
		{
			...
			//Verified By Double
			//如果先尝试检查一下是否一开始连续3~4个box的宽度都小于了平均字符宽度，我们就把
			//前三个（没有包含边框），前四个中的后三个（包含了边框）的box合并
			int wid = allBoxes[0].width;
			int box_idx = 0;
			for (; wid >= config->segmentationMinBoxWidthPx&&wid < avgCharWidth&&box_idx < 4;)
			{
				float DiffPixels = abs(wid - avgCharWidth);
				float DiffPercent = DiffPixels / avgCharWidth;
				if (DiffPercent > 0.6)
				{
					box_idx++;
					wid = allBoxes[box_idx].width;
				}
				else
					break;
			}
			// if (box_idx == 1)说明框到了车牌边上，并且认为是1，把它忽略了
			if (box_idx >2)//至少有3个小字符，box_idx=3或4
			{
				if (box_idx == 3)
				{
					Point botRight = Point(allBoxes[box_idx - 1].x + allBoxes[box_idx - 1].width, allBoxes[box_idx - 1].y + allBoxes[box_idx - 1].height);
					validBoxes.push_back(Rect(allBoxes[0].tl(), botRight));
				}
				if (box_idx == 4)
				{
					Point botRight = Point(allBoxes[box_idx - 1].x + allBoxes[box_idx - 1].width, allBoxes[box_idx - 1].y + allBoxes[box_idx - 1].height);
					validBoxes.push_back(Rect(allBoxes[1].tl(), botRight));
				}
			}
			...
			for (unsigned int boxidx = box_idx; boxidx < allBoxes.size(); boxidx++)
			{
