...
namespace alpr
{
  ...
  //q
  std::string recognizeColor(const Mat& img, AlprPlateResult& plates, bool show)
  {
	  ...
	  {
		  Point te;
		  te.x = plates.plate_points[k].x;
		  te.y = plates.plate_points[k].y;
		  coords.push_back(te);
	  }
	  cv::Point a, b, c, d;
	  ...
	  cv::Point top(minX, minY), bottom(maxX, maxY);

	  Point p3(maxX, minY);
	  Point p4(minX, maxY);

	  //          line(img, top, p3, Scalar(255,0,0), 2);
	  //          line(img, p3, bottom, Scalar(255,0,0), 2);
	  //          line(img, bottom, p4, Scalar(255,0,0), 2);
	  //          line(img, p4, top, Scalar(255,0,0), 2);
	  ...
	  int blacknum = 0;
	  int othernum = 0;
	  Mat carplate1 = img(Rect(minX, minY, maxX - minX, maxY - minY));
	  Mat carplate; carplate1.copyTo(carplate);
	  ...
	  for (int row = 0; row<hsv.rows; row++)
		  ...
		  {
			  //////////////////h通道/-/////////////////////
			  uchar hvalue = imgH.at<uchar>(row, col);  //
														//////////////////s通道/-/////////////////////
			  uchar svalue = imgS.at<uchar>(row, col);  //
														//////////////////v通道/-/////////////////////
			  uchar vvalue = imgV.at<uchar>(row, col);  //
														//printf("x=%d,y=%d,HSV: H=%d,S=%d,V=%d\n",row,col,hvalue,svalue,vvalue);
			  if ((hvalue>100 && hvalue<124) && (svalue>43 && svalue<225) && (vvalue>46 && vvalue<255))//hsv图像在蓝色区域里
			  {
				  //蓝色+1
				  bluenum++;
				  carplate.at<Vec3b>(row, col)[0] = 255;
				  carplate.at<Vec3b>(row, col)[1] = 0;
				  carplate.at<Vec3b>(row, col)[2] = 0;
			  }
			  else  if ((hvalue>11 && hvalue<34) && (svalue>43 && svalue<255) && (vvalue>46 && vvalue<255))//hsv在黄色区域里
			  {
				  //黄色+1
				  yellownum++;
				  carplate.at<Vec3b>(row, col)[0] = 0;
				  carplate.at<Vec3b>(row, col)[1] = 255;
				  carplate.at<Vec3b>(row, col)[2] = 255;
			  }
			  else if ((hvalue>0 && hvalue<180) && (svalue>0 && svalue<255) && (vvalue>0 && vvalue<60))//hsv在黑色区域里
			  {
				  blacknum++;
				  carplate.at<Vec3b>(row, col)[0] = 0;
				  carplate.at<Vec3b>(row, col)[1] = 0;
			  ...
			  else  if ((hvalue>0 && hvalue<180) && (svalue>0 && svalue<30) && (vvalue>150 && vvalue<255))//hsv在白色区域里
			  {
				  //白色+1
				  whitenum++;
				  carplate.at<Vec3b>(row, col)[0] = 255;
			  ...
			  else if ((hvalue>35 && hvalue<99) && (svalue>0 && svalue<30 || svalue>43 && svalue<255) && (vvalue>46 && vvalue<255))
			  {
				  //其他颜色
				  othernum++;
				  carplate.at<Vec3b>(row, col)[0] = 0;
  ...
  AlprFullDetails AlprImpl::recognizeFullDetails(cv::Mat img, std::vector<cv::Rect> regionsOfInterest)
  {
	...
	for (unsigned int i = 0; i < response.results.plates.size(); i++) {
		std::string color;
		//color = recognizeColor(img,response);
		color = recognizeColor(img, response.results.plates[i], config->debugGeneral && config->debugShowImages);
		response.results.plates[i].color = color;
  ...
  AlprFullDetails AlprImpl::analyzeSingleCountry(cv::Mat colorImg, cv::Mat grayImg, std::vector<cv::Rect> warpedRegionsOfInterest)
  {
    ...
    while(!plateQueue.empty())
    {
      ...
      if (!pipeline_data.disqualified)
      {
		...
		//Verified by Double
		pipeline_data.region_code = plateResult.region;
		//Verified by Double
