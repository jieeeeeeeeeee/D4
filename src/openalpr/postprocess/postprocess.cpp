...
namespace alpr
{
	PostProcess::PostProcess(Config* config)
	{
		...
		while (infile >> region >> pattern)
		{
			...
			//tempchar = Utf_8ToUnicode1(pattern.c_str());
			//resulttemp = UnicodeToAnsi1(tempchar);
			////strcpy(symbol, resulttemp);
			//pattern = resulttemp;
	...
	//Verified by Double
	void PostProcess::addLetter(string letter, int line_index, int charposition, float score, std::string templateregion)
	{
		wchar_t *tempchar;
		char * resulttemp;

		string letter_ansi = letter;
		int wcsLen = ::MultiByteToWideChar(CP_UTF8, NULL, letter_ansi.c_str(), strlen(letter_ansi.c_str()), NULL, 0);
		wchar_t* wszString = new wchar_t[wcsLen + 1];
		::MultiByteToWideChar(CP_UTF8, NULL, letter_ansi.c_str(), strlen(letter_ansi.c_str()), wszString, wcsLen);
		wszString[wcsLen] = '\0';
		tempchar = wszString;
		int nLen = WideCharToMultiByte(CP_ACP, 0, tempchar, -1, NULL, 0, NULL, NULL);
		if (nLen == 0)
		{
			letter_ansi[0] = NULL;
		}
		...
		//cout << "字符："<<letter_ansi<<" Index:"<<charposition << endl;
		...
		{
			//首个单词是中文
			if (charposition == 0 && letter_ansi.size()>1)
			{
				//如果置信度低于最小置信度，肯定也会小于skip_level
				if (score < min_confidence)
					score = skip_level + min_confidence + score - 100;
				else
			...
			if (templateregion == "sg" || templateregion == "jc" || templateregion == "ag" || templateregion == "jx")
			{
				//这些车牌的最后一位是汉字
				if (charposition == 6 && letter_ansi.length()>1)
				{
					if (score < min_confidence)
						score = skip_level + min_confidence + score - 100;
					else
			...
			//其他位置出现汉字都不加入
			if (templateregion != "wj"&&letter_ansi.length()>1)
				return;
